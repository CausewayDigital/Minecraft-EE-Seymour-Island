...
execute @e[type=armor_stand, tag=task_1] ~ ~ ~ detect ~0 ~5 ~5 bookshelf -1 setblock ~0 ~5 ~5 air

# If it still needs to be there, put the marker block back
execute @e[type=armor_stand, tag=task_1] ~ ~ ~ execute @p[x=~-9, y=~2, z=~-7, dx=15, dy=8, dz=13] ~ ~ ~ execute @e[type=armor_stand, tag=task_1, c=1] ~ ~ ~ setblock ~0 ~5 ~5 bookshelf 0
