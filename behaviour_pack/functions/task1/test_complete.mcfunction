# This function is run as the armor stand
...
execute @s ~ ~ ~ detect ~-5 ~3 ~-1 redstone_wire -1 execute @p[scores={task_1_rs_2_done=0}, x=~-9, y=~2, z=~-7, dx=15, dy=8, dz=13] ~ ~ ~ scoreboard players set @s task_1_rs_1_done 1

# Redstone 3 sound effect
execute @s ~ ~ ~ detect ~-6 ~3 ~-1  redstone_wire -1 execute @p[scores={task_1_rs_3_done=0}, x=~-9, y=~2, z=~-7, dx=15, dy=8, dz=13] ~ ~ ~ /playsound random.levelup @s ~ ~ ~ 100
execute @s ~ ~ ~ detect ~-6 ~3 ~-1  redstone_wire -1 execute @p[scores={task_1_rs_3_done=0}, x=~-9, y=~2, z=~-7, dx=15, dy=8, dz=13] ~ ~ ~ scoreboard players set @s task_1_rs_1_done 1
