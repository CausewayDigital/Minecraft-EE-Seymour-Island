...
execute @e[type=armor_stand, tag=crypto_vault] ~ ~ ~ execute @p[x=~-6, y=~5, z=~4, dx=12, dy=6, dz=-7, scores={crypto_v_done=0, crypto_v_test=0}] ~ ~ ~ execute @e[type=armor_stand, tag=crypto_vault, c=1] ~ ~ ~ detect ~ ~11 ~-3 wooden_button 11 function cryptovault/test_complete
