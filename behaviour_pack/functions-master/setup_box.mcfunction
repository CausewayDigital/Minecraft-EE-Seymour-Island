scoreboard players set @a[x=-878,y=120,z=383,r=2] task0 0


scoreboard players set @a[x=-878,y=120,z=383,r=2] task1 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task2 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task3 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task4 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task5 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task6 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task7 0
scoreboard players set @a[x=-878,y=120,z=383,r=2] task8 0
#scoreboard players reset * score

scoreboard players set @a[x=-878,y=120,z=383,r=2] score 0

scoreboard players set @a[x=-878,y=120,z=383,r=2] tick 0
execute @a[x=-878,y=120,z=383,r=2] ~ ~ ~ function misc/no_coding

title @a[x=-878,y=120,z=383,r=2] title Welcome to Seymour Island Map

tp @a[x=-878,y=120,z=383,r=2] -876 124 386 facing -876 124 388
